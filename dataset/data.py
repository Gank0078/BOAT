...
def read_txt_as_list(prefix, file_path):
    data_list = []
...
def get_cifar10(args, root):
    ...
    elif args.dist_mode == 1:
        u_samples = make_middle_data(args.num_max_u, args.num_classes, args.imb_ratio_unlabel)
    elif args.dist_mode == 2:
        u_samples = make_headtail_data(args.num_max_u, args.num_classes, args.imb_ratio_unlabel)
...
def make_middle_data(max_num, class_num, gamma):
    mu = np.power(1 / gamma, 1 / (class_num - 1))
    class_num_list = torch.zeros((class_num, ))
    class_num_list = class_num_list.long()
    class_pre = []
    for i in range(class_num):
        if i == (class_num - 1):
            class_pre.append(int(max_num / gamma))
        else:
            class_pre.append(int(max_num * np.power(mu, i)))

    cls_idx = 0
    idx1 = int((class_num - 1) / 2)
    idx2 = idx1 + 1
    while idx1 >= 0 or idx2 <= class_num - 1:
        if idx1 >= 0:
            class_num_list[idx1] = class_pre[cls_idx]
            cls_idx += 1
            idx1 -= 1

        if idx2 <= class_num - 1:
            class_num_list[idx2] = class_pre[cls_idx]
            cls_idx += 1
            idx2 += 1

    class_num_list = class_num_list.tolist()
    print(class_num_list)
    return list(class_num_list)


def make_headtail_data(max_num, class_num, gamma):
    mu = np.power(1 / gamma, 1 / (class_num - 1))
    class_num_list = torch.zeros((class_num, ))
    class_num_list = class_num_list.long()
    class_pre = []
...
class Places365(VisionDataset):
    def __init__(self, root, train=False, transform=None, target_transform=None, indexs=None, loader=default_loader):
        ...
        # self.path2label = read_txt_as_dict(root + '/categories_places365.txt')
...
def get_places365(args, root):
    ...
    base_dataset = Places365(root, True)

    labeled_ratio = 0.01
    arr_num_per_cls = np.load(os.path.join(root, 'num_per_cls.npy'))
    num_labeled_per_cls = [int(np.around(num * labeled_ratio)) for num in arr_num_per_cls]
    train_labeled_idxs = train_split_l(base_dataset.targets, num_labeled_per_cls, args)

    train_labeled_dataset = Places365(root, True, transform=transform_labeled,
                                          indexs=train_labeled_idxs)
    train_unlabeled_dataset = Places365(root, True, transform=TransformFixMatchPlaces365(mean=dataset_mean,
                                                                                                     std=dataset_std))
    test_dataset = Places365(root, False, transform=transform_val)

    return train_labeled_dataset, train_unlabeled_dataset, test_dataset


def get_food101(args, root):
    resize_dim = 256
    crop_dim = 224
    dataset_mean = (0.485, 0.456, 0.406)
    dataset_std = (0.229, 0.224, 0.225)
    ...
    base_dataset = datasets.Food101(root=root, split='train', transform=None)
    labeled_ratio = 0.1
    num_labeled_per_cls = [int(np.around(750 * labeled_ratio)) for _ in range(args.num_classes)]
    train_labeled_idxs = train_split_l(base_dataset._labels, num_labeled_per_cls, args)

    train_labeled_dataset = datasets.Food101(root=root, split='train', transform=transform_labeled)
    ...
    train_unlabeled_dataset = datasets.Food101(root=root, split='train',
                                               transform=TransformFixMatch(mean=dataset_mean, std=dataset_std))
    test_dataset = datasets.Food101(root=root, split='test', transform=transform_val)
    # print('-')
    return train_labeled_dataset, train_unlabeled_dataset, test_dataset
...
def get_imagenet(args, root):
    ...
    dataroot = '/mnt/cc52dc96-f743-4ded-8bcf-02185c316a81/gank/datasets/ImageNet'

    # base_dataset = ImageNet(root=cfg.DATA.DATAPATH, train=True)
    base_dataset = ImageNet(root=dataroot, train=True)

    labeled_ratio = 0.01
    # num_per_cls = np.load(cfg.DATA.DATAPATH+'/num_per_cls.npy')
    num_per_cls = np.load(dataroot + '/num_per_cls.npy')
    num_per_cls = num_per_cls.tolist()
    num_labeled_per_cls = [int(np.around(num_per_cls[idx] * labeled_ratio)) for idx in range(args.num_classes)]
    train_labeled_idxs = train_split_l(base_dataset.targets, num_labeled_per_cls, args)

    train_labeled_dataset = ImageNet(root=dataroot, train=True, transform=transform_labeled,
                                                 indexs=train_labeled_idxs)

    train_unlabeled_dataset = ImageNet(root=dataroot, train=True,
                                                   transform=TransformFixMatch_ws(mean=dataset_mean, std=dataset_std))

    test_dataset = ImageNet(root=dataroot, train=False, transform=transform_val)
